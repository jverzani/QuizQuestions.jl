...
function Base.show(io::IO, m::MIME"text/latex", x::Question)
    _show_latex_label(io, m, x)
end

LATEXBOX = raw"${\quad\Box}$ "
function Base.show(io::IO,  m::MIME"text/latex", x::Union{Radioq, Buttonq})
    _show_latex_label(io, m, x)
...
function Base.show(io::IO,  m::MIME"text/latex", x::Union{Multiq, MultiButtonq})
    _show_latex_label(io, m, x)
    for l ∈ x.labels
        print(io, LATEXBOX)
        show(io, m, Markdown.parse(l))
...
function Base.show(io::IO,  m::MIME"text/latex", x::Matchq)
    _show_latex_label(io, m, x)
...
function Base.show(io::IO,  m::MIME"text/latex", x::FillBlankQ)
    _show_latex_label(io, m, x)
    q = x.question
...
function Base.show(io::IO,  m::MIME"text/latex", x::HotspotQ)
    _show_latex_label(io, m, x)
    println(io, "insert image here")
end

function Base.show(io::IO,  m::MIME"text/latex", x::PlotlyLightQ)
    _show_latex_label(io, m, x)
    println(io, "insert image here")
end
